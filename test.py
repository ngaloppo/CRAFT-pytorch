...
def post_process(
    score_text, score_link, text_threshold, link_threshold, low_text, poly, target_ratio
):
    # Post-processing
...
def test_net_openvino(
    ...
    """Perform inference with an image (RGB, HWC) input"""
    # resize
    ...
    x = np.expand_dims(np.transpose(img_resized, (2, 0, 1)), axis=0)
    y = net(x)[net.outputs[0]]
...
if __name__ == "__main__":
    ...
    if args.trained_model.endswith(".xml"):
        ...
        refine_net = None
        assert not args.refine

        test_net = test_net_openvino
    else:
        # load net
    ...
    for k, image_path in enumerate(image_list):
        ...
        image = imgproc.loadImage(image_path)  # RGB, HWC
